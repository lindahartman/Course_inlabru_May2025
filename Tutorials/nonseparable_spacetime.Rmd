...
author: "Elias T Krainski, with updates from Finn Lindgren"
...
cache_path <- "nonseparable_spacetime_cache"
knitr::opts_chunk$set(
  echo = TRUE,
  warning = FALSE,
  cache = TRUE,
  cache.path = paste0(cache_path, "/"),
  collapse = TRUE,
  comment = "#>",
  dev = "png",
...
Download and read the borders file, station coordinates, and observation data
to a cache directory, e.g. `cache_path <- "``r cache_path``"`:
```{r downloadporders}
...
Prepare the time to be used (alternatively, a different time mapping can be used)
```{r time}
...
With **inlabru** we can define the observation/likelihood model with 
the `bru_obs()` function and use it for fitting models with
...
which is not needed but helps model components identification.
...
around $203.7$ by $266.5$ kilometres, which we already did when building the 
...
We can compare our results with Table 3 in @cameletti2013 with
